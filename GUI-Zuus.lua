...
function BTW.Combo()
	...
	for i = 1, Length(ordercast) do
		BTW.Cast(ordercast[i], BTW.Hero, enemy, Entity.GetAbsOrigin(enemy), BTW.MP)
	end

	if not NPC.HasModifier(enemy, "modifier_item_ethereal_blade_slow") and GUI.IsEnabled(BTW.Identity .. "orbwalker") then
		Player.PrepareUnitOrders(Players.GetLocal(), 4, enemy, Vector(0,0,0), nil, Enum.PlayerOrderIssuer.DOTA_ORDER_ISSUER_PASSED_UNIT_ONLY, BTW.Hero)
	end
end
		...
		if not Entity.IsDormant(npc) and Wrap.EIsAlive(npc) and not NPC.IsIllusion(npc) and not Entity.IsSameTeam(BTW.Hero, npc) and not NPC.HasState(npc, Enum.ModifierState.MODIFIER_STATE_MAGIC_IMMUNE) then
			...
			if isInterrupt and NPC.IsChannellingAbility(npc) or NPC.HasModifier(npc, "modifier_teleporting") then
				...
				elseif Ability.IsCastable(BTW.Abilitys['zuus_cloud'], BTW.MP) then
					BTW.Cast('zuus_cloud', BTW.Hero, npc, Entity.GetAbsOrigin(npc), BTW.MP)
				end
			end
